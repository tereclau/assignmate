  ...
  return (
    <div className="max-w-6xl mx-auto py-8">
      <div className="flex items-center gap-4 mb-12">
        <button 
          onClick={onBack}
          className="w-10 h-10 rounded-xl bg-white/50 border border-white/40 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-white transition-all"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div>
          <h2 className="text-3xl font-display font-bold text-slate-900 tracking-tight">Upgrade ke Premium</h2>
          <p className="text-slate-500 font-medium italic">Pilih paket yang sesuai dengan gaya belajarmu.</p>
        </div>
      </div>
          ...
          >
            {PLANS.map((plan) => (
              <div 
                key={plan.id}
                className={cn(
                  "glass p-10 rounded-[2.5rem] relative flex flex-col group transition-all duration-500 hover:scale-[1.02]",
                  plan.isPopular ? "border-indigo-500 ring-2 ring-indigo-500 ring-offset-4 ring-offset-transparent" : "border-white/40"
                )}
              >
                {plan.isPopular && (
                ...
                <button 
                  ...
                  className={cn(
                    "w-full py-4 rounded-2xl font-bold transition-all uppercase tracking-widest text-xs",
                    plan.isPopular 
